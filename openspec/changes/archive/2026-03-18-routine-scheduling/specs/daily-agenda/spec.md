...
The system SHALL display a daily agenda view that shows routines with assigned times as chronologically ordered time blocks.

#### Scenario: View agenda with scheduled routines
- **WHEN** the user opens the agenda view and has routines with start/end times (e.g., "Meditar" 8:00-8:30, "Ejercicio" 7:00-7:45)
...
- **WHEN** the user has some routines with times and some without
...
- **WHEN** the user has no routines with assigned times
